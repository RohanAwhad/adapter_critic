"""
v11: served-adapter with its_hub best-of-n + gpt-oss-120b as llm-judge.

Starts two servers:
  - its_hub IaaS on port 8108 (best-of-n with llm-judge using gpt-oss-120b)
  - adapter_critic on port 8000 (served-adapter wired to IaaS for adapter stage)

Requires:
  - localhost:8100 running qwen3-30b-a3b-thinking (generation)
  - localhost:8101 running gpt-oss-120b (judge)
"""
...
from experiments.v11.upstream_resolution import (
...
def configure_iaas(model_base_url: str, judge_base_url: str) -> None:
    """Configure its_hub IaaS with best-of-n + gpt-oss-120b as llm-judge."""
    its_cfg = {
        "provider": "openai",
    ...
    logger.info("its_hub configured: best-of-n + llm-judge (judge={} on {})", JUDGE_MODEL, judge_base_url)
...
def main() -> None:
    ...
    model_base_url = build_upstream_base_url(host=upstream_host, port=8100)
    judge_base_url = build_upstream_base_url(host=upstream_host, port=8101)
    ...
    # 2. Configure IaaS
    configure_iaas(model_base_url, judge_base_url)
