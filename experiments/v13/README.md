# V13 Experiment: Vertex Claude API + Qwen3 interventionist

## What this experiment is

`experiments/v13/` runs `adapter_critic` with:

- `served-direct`: Vertex Anthropic Claude only
- `served-adapter`: Vertex Anthropic Claude API draft + Qwen3 interventionist
...
- interventionist target (`served-adapter.adapter`):
  ...
  - base URL: `http://<UPSTREAM_HOST>:8100/v1` (`UPSTREAM_HOST` defaults to `localhost`)
...
3. Qwen3 OpenAI-compatible endpoint is running on `http://<UPSTREAM_HOST>:8100/v1`.
...
uv run python -m experiments.v13.run_server
```
