# V2 Experiment Server

## What this experiment is

`experiments/v2/` is a local playground for running `adapter_critic` with file-based adapter and critic system prompts.
...
```bash
UPSTREAM_HOST=host.docker.internal uv run python -m experiments.v2.run_server
```

Or default to localhost:

```bash
uv run python -m experiments.v2.run_server
```
