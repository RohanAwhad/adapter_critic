# V3 Experiment Server

## What this experiment is

`experiments/v3/` is a local playground for running `adapter_critic` with file-based adapter and critic system prompts.
...
```bash
UPSTREAM_HOST=host.docker.internal uv run python -m experiments.v3.run_server
```

Or default to localhost:

```bash
uv run python -m experiments.v3.run_server
```
