# V4 Experiment Server

## What this experiment is

`experiments/v4/` is a local playground for running `adapter_critic` with file-based adapter and critic system prompts.
...
```bash
UPSTREAM_HOST=host.docker.internal uv run python -m experiments.v4.run_server
```

Or default to localhost:

```bash
uv run python -m experiments.v4.run_server
```
