# V7 Experiment Server

## What this experiment is

`experiments/v7/` is a local playground for running `adapter_critic` with file-based adapter and critic system prompts.
...
- `served-direct` -> API model at `http://<upstream-host>:8101/v1` (`gpt-4.1`)
- `served-adapter` ->
  - API draft model at `http://<upstream-host>:8101/v1` (`gpt-4.1`)
  - adapter model at `http://<upstream-host>:8100/v1` (`glm-4.7-flash`)
- `served-critic` ->
  - API draft + second pass model at `http://<upstream-host>:8101/v1` (`gpt-4.1`)
  - critic model at `http://<upstream-host>:8100/v1` (`glm-4.7-flash`)
...
```bash
UPSTREAM_HOST=host.docker.internal uv run python -m experiments.v7.run_server
```

Or default to localhost:

```bash
uv run python -m experiments.v7.run_server
```
